  ...
  const aggregateResults = (results: SubnetQueryResult[]) => {
    const ipMap = new Map<
      string,
      {
        ...
        ttl: number
        name: string
        sources: Array<{ country: string; region: string; province: string; isp: string; subnet: string }>
      }
    >()
  ...
  return (
    <div className="min-h-screen bg-background py-16">
      <div className="max-w-7xl mx-auto px-4">
        <header className="text-center mb-8">
          ...
          {systemStatus?.digAvailable === false && (
            <div className="mt-4 space-y-2">
              <Badge
                variant={systemStatus.digAvailable ? 'default' : 'destructive'}
                className="inline-flex items-center gap-2"
              >
                {systemStatus.digAvailable ? <CheckCircle2 className="h-3 w-3" /> : <AlertCircle className="h-3 w-3" />}
                &apos;DIG工具路径设置错误，未找到工具&apos;
              </Badge>
            </div>
          )}
        ...
        {loading && <MultiQueryResultSkeleton />}
                  ...
                  {result.parsed.answer && result.parsed.answer.length > 0 && (
                    <div>
                      ...
                      <div className="border rounded-lg bg-background">
                        <div className="p-3">
                          <div className="space-y-2">
                            {result.parsed.answer.map(
                              (record: { name: string; type: string; ttl: number; rdata: string }, index: number) => (
                ...
                <Tabs defaultValue="results" className="w-full">
                  ...
                  <TabsContent value="results" className="space-y-4">
                    ...
                    {filteredResults.successfulResults.length > 0 ? (
                      <div className="border rounded-lg bg-background">
                        <div className="p-3 bg-zinc-500/10">
                          <div className="space-y-3">
                            {aggregateResults(filteredResults.successfulResults).map(
                              (
                                ipResult: {
                                  ip: string
                                  type: string
                                  ttl: number
                                  sources: Array<{ country: string; province: string; isp: string }>
                                },
                                index: number
                              ) => (
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-2">
                                    <div className="max-w-[200px] flex flex-col gap-1">
                                      <div>
                                        ...
                                        <a className="font-mono text-muted-foreground">{ipResult.ip}</a>
                                      </div>
                                      <div>
                                        <span className="font-medium">TYPE:</span>{' '}
                                        <a className="font-mono text-muted-foreground">{ipResult.type}</a>
                                      </div>
                                      <div>
                                        <span className="font-medium">TTL:</span>{' '}
                                        <a className="font-mono text-muted-foreground">{ipResult.ttl}</a>
                                      </div>
                                    </div>
                                    <div className="col-span-3 flex flex-col">
                                      <span className="font-medium">{ipResult.sources.length} 个来源:</span>
                                      <div className="mt-1 flex flex-wrap gap-1">
                                        {ipResult.sources.map(
                                          (
                                            source: { country: string; province: string; isp: string },
                                            sourceIndex: number
                                          ) => (
                      ...
                      <div className="text-center py-8 text-muted-foreground">暂无解析结果</div>
                    )}
                  </TabsContent>
                  ...
                  <TabsContent value="status" className="space-y-4">
                    ...
                    <div className="border rounded-lg bg-background">
                      <div className="p-6">
                          ...
                          <TableBody>
                              ...
                              <TableRow key={index}>
                                ...
                                <TableCell className="max-w-60">
                                  <div className="bg-muted rounded p-2 max-h-40 overflow-x-auto overflow-y-auto">
                                    <pre className="text-xs">{item.result.output}</pre>
                                  </div>
                                </TableCell>
                              </TableRow>
                            ))}
                            {filteredResults.failedResults.map((item, index) => (
                              <TableRow key={`failed-${index}`}>
                                ...
                                <TableCell>-</TableCell>
                                <TableCell>
                                  <div className="text-destructive text-xs">ERROR: {item.error}</div>
                                </TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        ...
                        {filteredResults.successfulResults.length === 0 &&
                          filteredResults.failedResults.length === 0 && (
                            <div className="text-center py-8 text-muted-foreground">没有符合筛选条件的结果</div>
                          )}
                      </div>
                    </div>
                  </TabsContent>
