...
export default function Home() {
  ...
  const aggregateResults = (results: SubnetQueryResult[]) => {
    ...
    results.forEach(({ result, subnetInfo }) => {
      if (result.parsed.answer) {
        result.parsed.answer.forEach((record) => {
  ...
  const filteredResults = useMemo(() => {
    ...
    const filterItem = (item: SubnetQueryResult | FailedSubnetQueryResult) => {
      ...
      if (ipSearch && 'result' in item) {
        const hasMatchingIP = item.result.parsed.answer?.some((record) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'DNS查询失败')
    } finally {
      setLoading(false)
    }
  }
                  ...
                  {result.parsed.answer && result.parsed.answer.length > 0 && (
                    <div>
                      ...
                      <div className="border rounded-lg bg-background">
                        <div className="p-3">
                          <div className="space-y-2">
                            {result.parsed.answer.map((record, index: number) => (
                  ...
                  {result.parsed.authority && result.parsed.authority.length > 0 && (
                    <div>
                      ...
                      <div className="border rounded-lg bg-background">
                        <div className="p-3">
                          <div className="space-y-2">
                            {result.parsed.authority.map((record, index: number) => (
                              <div key={index} className="bg-muted rounded p-2 text-sm">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                  <div>
                                    <span className="font-medium">域名:</span>
                      ...
                      <div className="border rounded-lg bg-background">
                        <div className="p-3 bg-zinc-500/10">
                          <div className="space-y-3">
                            {aggregateResults(filteredResults.successfulResults).map((ipResult, index: number) => (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-2">
                                  ...
                                  <div className="col-span-3 flex flex-col">
                                    ...
                                    <div className="mt-1 flex flex-wrap gap-1">
                                      {ipResult.sources.map((source, sourceIndex: number) => (
